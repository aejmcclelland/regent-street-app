...
export default async function OrganisationsGroupPage({
    params,
}: {
    ...
    const displayData = await resolveLinkedPage({
        primaryCollection: 'organisations',
        slug,
    });
                        ...
                        <div className='mt-10'>
                            <ul className='menu menu-vertical lg:menu-horizontal menu-lg bg-base-200 rounded-box'>
                                {displayData.subgroups.map((sub: any) => (
                                    <li key={sub.slug}>
                                        <Link href={`/whats-on/organisations/${sub.slug}`}>
                                            {sub.name}
                                        </Link>
                                    </li>
                                ))}
